...
async def check_status_of_not_started_week(schedule_plans, schedule, week, message, week_index):
    ...
    if has_week_started:
        ...
        await safe_send(message.channel, f'Week has started for league with context {schedule["context"]} for season {schedule["season"]}')
        await notify_staff_for_matches(message, schedule)
    else:
        await safe_send(message.channel, f'Week has not started yet for league with context {schedule["context"]} for season {schedule["season"]}')
        return
