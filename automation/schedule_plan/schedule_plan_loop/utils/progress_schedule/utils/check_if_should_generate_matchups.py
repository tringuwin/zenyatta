...
async def check_if_should_generate_matchups(message, db, schedule):
    ...
    if not matchups_config['auto_generate']:
        await safe_send(message.channel, constants.STAFF_PING+' Cannot automatically generate matchups for schedule plan with context '+schedule['context']+' and season '+str(schedule['season']))
        return
