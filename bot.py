...
def is_valid_channel(message, lower_message, is_helper, is_push_bot):
    ...
    if lower_message == '!hello' or lower_message == '!gg ez' or lower_message.startswith('!whichhero') or lower_message.startswith('!whichhero') or lower_message=='!pingteam' or lower_message.startswith('!profile'):
        return True, None
...
async def handle_message(message, db, client):
    ...
    if lower_message == '!help':
        ...
        response = 'Spicy OW is the **61nd** highest ranked Overwatch Discord Server'
        response += '\nNext Server to Beat: **Brigitte Mains**'
