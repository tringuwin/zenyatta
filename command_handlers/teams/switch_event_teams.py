...
async def switch_event_teams(db, message):
    ...
    if len(word_list) < 5:
        await safe_send(message.channel, 'Not enough params')
        return

    event_id = word_list[1]
    event = get_event_by_id(db, event_id)
    if not event:
        await safe_send(message.channel, 'Invalid event id')
        return
    
    bracket = await get_bracket_by_event_id(db, event_id)
    if not bracket:
        await safe_send(message.channel, 'Bracket does not exist for this event')
        return
    ...
    if not new_team:
        await safe_send(message.channel, 'No team with that name exists')
        return
