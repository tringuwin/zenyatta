...
async def invite_handler(db, message, is_admin):
    ...
    if team['creator_id'] != user['discord_id'] and (not is_admin):
        await safe_send(message.channel, 'You are not the owner of this team.')
        return
    ...
    if len(mentions) != 1:
        await safe_send(message.channel, 'Please mention 1 player to invite to the team')
        return
    
    invited_member = mentions[0]
    invited_user = user_exists(db, invited_member.id)
    if not invited_user:
        await safe_send(message.channel, 'That user is not registered. Please have them register with this command: **!battle BattleTagHere#1234**')
        return

    if user_on_team(team, invited_user['discord_id']):
        await safe_send(message.channel, 'That user is already on this team.')
        return

    if user_invited_to_team(team, invited_user):
        await safe_send(message.channel, 'This user has already been invited to this team.')
        return
    
    await invite_user_to_team(db, team, invited_user)
    add_invite_to_team(db, team, invited_user['discord_id'])
    await safe_send(message.channel, 'User was successfully invited to the team! '+invited_member.mention+' to accept this invite, please say **!acceptinvite '+team['team_name']+'**')