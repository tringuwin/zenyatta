...
async def cancel_ally_handler(db, message, context):
    ...
    if not (my_team_obj['team_name'] in other_team_obj['ally_reqs']):
        await safe_send(message.channel, 'Your team did not send an Ally Request to '+other_team_obj['team_name'])
        return
    ...
    await safe_send(message.channel, 'Ally Request successfully cancelled.')