...
async def cancel_rival_handler(db, message, context):
    ...
    if not (my_team_obj['team_name'] in other_team_obj['rival_reqs']):
        await safe_send(message.channel, 'Your team did not send a Rival Request to '+other_team_obj['team_name'])
        return
    ...
    await safe_send(message.channel, 'Rival Request successfully cancelled.')