...
async def deny_rival_handler(db, message):
    ...
    if not (other_team_obj['team_name'] in my_team_obj['rival_reqs']):
        await message.channel.send('Your team does not have a Rival Request from '+other_team_obj['team_name'])
        return
    ...
    await message.channel.send('Rival Request successfully denied.')