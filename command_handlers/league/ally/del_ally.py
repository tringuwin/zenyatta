...
async def del_ally_handler(db, message, client, context):
    ...
    if not (other_team_obj['team_name'] in my_team_obj['allies']):
        await safe_send(message.channel, 'This team is not an Ally of your team.')
        return
    ...
    await safe_send(league_notifs_channel, my_team_emoji_string+' '+team_name+' and '+other_team_emoji_string+' '+other_team_obj['team_name']+' are no longer Allies.')
