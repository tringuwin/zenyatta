...
async def rival_request_handler(db, message, context):
    ...
    if team_name.lower() == team_name_to_req:
        await safe_send(message.channel, 'You cannot send a Rival Request to your own team.')
        return
    ...
    if team_name in other_team_obj['rival_reqs']:
        await safe_send(message.channel, 'This team already has a Rival Request from '+team_name+'.')
        return
    ...
    if team_name in other_team_obj['allies']:
        await safe_send(message.channel, 'This team is currently an Ally of '+team_name+'. Remove them as an Ally to send a Rival request.')
        return
    ...
    await safe_send(message.channel, 'You have successfully sent an Rival Request from '+team_name+' to '+other_team_obj['team_name']+'!')
