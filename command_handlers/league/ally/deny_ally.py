...
async def deny_ally_handler(db, message, context):
    ...
    if not (other_team_obj['team_name'] in my_team_obj['ally_reqs']):
        await safe_send(message.channel, 'Your team does not have an Ally Request from '+other_team_obj['team_name'])
        return
    ...
    await safe_send(message.channel, 'Ally Request successfully denied.')