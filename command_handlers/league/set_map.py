...
async def set_map_handler(db, message):
    ...
    if not can_be_int(map_num):
        await safe_send(message.channel, map_num+' is not a valid integer')
        return
    
    map_num = int(map_num)
    if map_num > 7 or map_num < 1:
        await safe_send(message.channel, 'Map num must be between 1 and 7')
        return

    map_name = parts[2]
    if not map_name in constants.MAP_NAME_TO_IMAGE:
        await safe_send(message.channel, map_name+' is not a valid map name')
        return
    ...
    await safe_send(message.channel, 'Map updated')