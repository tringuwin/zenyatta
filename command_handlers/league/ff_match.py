...
async def ff_match_handler(db, message, context):
    ...
    if not ff_team_data:
        await safe_send(message.channel, f'Team {ff_team} not found in the league context {context}.')
        return
    ...
    if not season_standings:
        await safe_send(message.channel, f'No standings found for the context {context} and season {current_season}.')
        return
    ...
    await safe_send(message.channel, f'{ff_team_name} has forfeited a match. Forfeits: {standings_forfeits[ff_team_name]}')
