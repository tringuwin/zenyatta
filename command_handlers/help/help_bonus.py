...
async def help_bonus_handler(message):
    ...
    help_embed.add_field(name='!hello', value='Say hi to the Giselle bot', inline=False)
    help_embed.add_field(name='!gg ez', value='Giselle will respond with one of the classic "gg ez" responses.', inline=False)
    help_embed.add_field(name='!whichhero [question]', value='Ask the Giselle bot a question and it will respond with a hero. (Example: !whichhero should be nerfed?)', inline=False)
