...
async def help_league_handler(message):
    ...
    safe_add_field(help_embed, '!invites', 'See a list of league teams that you are invited to.', False)
    safe_add_field(help_embed, '!accept [team name]', 'Accept an invite to join a league team.', False)
    safe_add_field(help_embed, '!deny [team name]', 'Deny an invite to join a league team.', False)
    safe_add_field(help_embed, '!leave', 'Leave your current league team.', False)
