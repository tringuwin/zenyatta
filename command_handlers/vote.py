...
async def vote_handler(db, message, client):
    ...
    if not twitch_sub_role in message.author.roles:
        await message.channel.send('Only Twitch Subscribers are able to vote.')
        return
    
    current_vote = get_constant_value(db, 'sub_vote')
    if not current_vote['active']:
        await message.channel.send('There is no vote at the moment.')
        return

    if message.author.id in current_vote['voted_users']:
        await message.channel.send('You have already voted in this vote.')
        return
    ...
    if not can_be_int(vote_option):
        await message.channel.send(vote_option+' is not a valid number.')
        return
    vote_option = int(vote_option)
    
    num_options = len(current_vote['options'])
    if (vote_option < 0) or vote_option > num_options:
        await message.channel.send('That number is not a valid option for this vote.')
        return
