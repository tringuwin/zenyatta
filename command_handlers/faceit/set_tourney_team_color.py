...
async def set_tourney_team_color(db, message):
    ...
    if not can_be_int(team_number):
        await safe_send(message.channel, team_number+" is not an integer")
        return
    team_number = int(team_number)
    
    if team_number != 1 and team_number != 2:
        await safe_send(message.channel, "Team number must be either 1 or 2")
        return
    
    team_color = command_parts[2].lower()
    if team_color not in VALID_COLORS:
        await safe_send(message.channel, "Invalid color. Valid colors are: "+", ".join(VALID_COLORS))
        return
    ...
    await safe_send(message.channel, "Team "+str(team_number)+" color set to "+team_color)
