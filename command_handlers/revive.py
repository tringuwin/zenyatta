...
async def revive_handler(db, message):
    ...
    if user_slimed:
        await safe_send(message.channel, "You have already been slimed. You cannot revive anyone unless you are revived first.")
        return
    ...
    if not mentioned_user_slimed:
        await safe_send(message.channel, "That user has not been slimed, so they don't need to be revived.")
        return
    
    if mentioned_member == message.author:
        await safe_send(message.channel, "You cannot revive yourself.")
        return
    
    user_gems = get_user_gems(user)
    white_gems = user_gems['white']
    if white_gems < 1:
        await safe_send(message.channel, "You need a white gem to revive someone.")
        return
    ...
    await safe_send(message.channel, f"{message.author.mention} has revived {mentioned_member.mention} by using a white gem!")